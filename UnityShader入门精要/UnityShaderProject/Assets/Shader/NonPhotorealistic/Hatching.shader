...
///
Shader "MyShaders/Hatching" {
	Properties {
		_Color ("Color Tint", Color) = (1, 1, 1, 1)		// 控制模型颜色
		_TileFactor ("Tile Factor", Float) = 1			// 纹理平铺属性，_TileFactor越大，模型上的素描线条越密
	...
	SubShader {
		...
		// 使用名为MyShaders/Toon Shading/OUTLINE的Pass
		UsePass "MyShaders/Toon Shading/OUTLINE"
		...
		Pass {
			...
			v2f vert(a2v v) {
				...
				if (hatchFactor > 6.0) {
					...
					// 两者纹理直接直接拼接会有点生硬，所以当我们选择在两张纹理之间进行过度
